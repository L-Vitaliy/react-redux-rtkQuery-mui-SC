      ...
      transformResponse(response: News.SnippetResponse<NewsType>[]): NewsEntity.Card<NewsType>[] {
        return response.map(
          (newsCard): NewsEntity.Card<NewsType> => ({
            id: newsCard.id,
            newsType: newsCard.news_type,
            header: newsCard.header,
            date: getClientShortDate(newsCard.date_publish),
            answers: newsCard.answers,
            categoryName: newsCard.roles?.[0]?.name || CONTENT_ROLE_DEFAULT,
            commentsCount: newsCard.comments,
            isImportant: newsCard.important_flag,
            forAllFlag: newsCard.for_all_flag,
        ...
        return {
          ...
          items: response.items.map(
            (newsCard): NewsEntity.Card<NewsType> => ({
              ...
              isImportant: newsCard.important_flag,
              forAllFlag: newsCard.for_all_flag,
              freeAnswerFlag: newsCard.free_answer_flag || false,
              manyChoiceFlag: newsCard.many_choice_flag || false,
              hideResultsFlag: newsCard.hide_results_flag || false,
              roles: newsCard.roles || [],
            }),
          ),
        };
